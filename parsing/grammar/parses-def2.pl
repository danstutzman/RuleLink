...
parses(eng, n, rule0, []) --> [mary].
parses(eng, n, rule1, []) --> [john].
parses(eng, v, rule2, []) --> [works].
parses(eng, v, rule3, []) --> [runs].
parses(eng, s, rule4, [[n, List1, List2], [v, List2, List3]]) -->
  parses(eng, n, _, []), parses(eng, v, _, []).
...
%parses(eng, s, rule4, [[n, List1, List2], [v, List2, List3]], List1, List3) :-
...
parses(esp, s, rule4, [[n, List1, List2], [v, List2, List3]], List1, List3) :-
	parses(esp, n1, _, [], List1, List2),
	parses(esp, v, _, [], List2, List3).

translates(EnglishFirst, EnglishRest, SpanishFirst, SpanishRest) :-
	parses(eng, _, MatchingRule, EnglishConsts, EnglishFirst, EnglishRest),
	parses(esp, _, MatchingRule, SpanishConsts, SpanishFirst, SpanishRest),
